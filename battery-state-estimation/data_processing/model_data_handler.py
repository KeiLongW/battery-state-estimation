...
class ModelDataHandler():
    ...
    def get_discharge_whole_cycle_future(self):
        ...
        current_train, current_test = self.__get_padded_whole_cycle(
            train_raw_x, test_raw_x)
        current_train = np.negative(current_train[:, :, 1])
        current_test = np.negative(current_test[:, :, 1])
        ...
        battery_name_cycle_train = self.train_discharge_cap[:, [
            CapacityCols.TEST_NAME]]
        battery_name_step_train = np.repeat(
            battery_name_cycle_train[:, None, :], train_x.shape[1], axis=1)
        battery_name_cycle_test = self.test_discharge_cap[:, [
            CapacityCols.TEST_NAME]]
        battery_name_step_test = np.repeat(
            battery_name_cycle_test[:, None, :], train_x.shape[1], axis=1)

        time_train, _ = self.__get_whole_cycle_soh_x_y(
            self.train_discharge_cyc, self.train_discharge_cap, [
                CycleCols.STEP_TIME], y_indices
        )
        time_test, _ = self.__get_whole_cycle_soh_x_y(
            self.test_discharge_cyc, self.test_discharge_cap, [
                CycleCols.STEP_TIME], y_indices
        )
        time_train, time_test = self.__get_padded_whole_cycle(
            time_train, time_test)

        self.logger.info('''Train x: %s, train y soh: %s | Test x: %s, test y soh: %s | 
                            battery names cycle train: %s, battery names steps train: %s, 
                            battery names cycle test: %s, battery names steps test: %s |
                            time train: %s, time test: %s |
                            raw current train: %s, raw current test: %s |
                            ''' %
                         (train_x.shape, train_y.shape, test_x.shape, test_y.shape,
                          battery_name_cycle_train.shape, battery_name_step_train.shape,
                          battery_name_cycle_test.shape, battery_name_step_test.shape,
                          time_train.shape, time_test.shape,
                          current_train.shape, current_test.shape))

        return (train_x, train_y, test_x, test_y,
                battery_name_cycle_train, battery_name_step_train,
                battery_name_cycle_test, battery_name_step_test,
                time_train, time_test,
                current_train, current_test)

    def prepare_y_future(self, battery_name_cycle, battery_name_step, y_soh, current, time, capacity_threshold):
        battery_name_set = set(battery_name_cycle.flatten().tolist())
        battery_range_cycle = [np.where(battery_name_cycle == x)[
            0][-1]+1 for x in battery_name_set]
        battery_range_cycle.sort()
        _, battery_count_cycle = np.unique(
            battery_name_cycle, return_counts=True)
        battery_name_step = battery_name_step.flatten()
        battery_range_step = [np.where(battery_name_step == x)[
            0][-1] for x in battery_name_set]
        battery_range_step.sort()
        self.logger.info("battery ranges: {}".format(battery_range_cycle))

        battery_nominal_capacity = [
            float(name.split("-")[2]) for name in battery_name_set]

        flattened_current = current.flatten()
        flattened_time = time.flatten()
        capacity_integral_train = []
        a = 0
        for battery_index, b in enumerate(battery_range_step):
            self.logger.info("processing range {} - {}".format(a, b))
            integral = [np.trapz(y=flattened_current[a:i],
                                 x=flattened_time[a:i]) for i in range(a, b)]
            integral = [x/battery_nominal_capacity[battery_index]
                        for x in integral]
            capacity_integral_train.extend(integral)
            a = b
        capacity_integral_train = capacity_integral_train[::current.shape[1]]
        capacity_integral_train = np.array(capacity_integral_train)
        self.logger.info("Train integral: {}".format(
            capacity_integral_train.shape))

        y_future = []
        a = 0
        for b in battery_range_cycle:
            self.logger.info("processing range {} - {}".format(a, b))
            index = np.argmax(y_soh[a:b] < capacity_threshold) + a
            self.logger.info("threshold index: {}".format(index))
            for i in range(a, b):
                y = capacity_integral_train[index] - \
                    capacity_integral_train[i] if i < index else 0
                y_future.append(y)
            a = b
    ...
    class NormalizationY():
        def normalize_y(self, train_y, test_y):
            self.min = [min(train_y[:, i]) for i in range(train_y.shape[1])]
            self.max = [max(train_y[:, i]) for i in range(train_y.shape[1])]
            for i in range(train_y.shape[1]):
                train_y[:, i] = (train_y[:, i] - self.min[i]) / \
                    (self.max[i] - self.min[i])
                test_y[:, i] = (test_y[:, i] - self.min[i]) / \
                    (self.max[i] - self.min[i])
            return (train_y, test_y)

        def y_to_plot(self, y):
            for i in range(y.shape[1]):
                y[:, i] = y[:, i] * (self.max[i] - self.min[i]) + self.min[i]
            return y
